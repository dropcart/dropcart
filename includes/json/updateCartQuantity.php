...
$objCart->updateProductQuantity($intCartId, $intQuantity);
...
while ($objCart = $objDB->getObject($result_cart)) {
    ...
    if ($objCart->id == $intCartId) {
        ...
        $dblCurrentProductPrice = $dblPrice;
        $dblCurrentProductTotal = $dblPrice * $intQuantity;
...
$arrOutput['productPrice'] = moneyFormat('%(#1n', $dblCurrentProductPrice);
$arrOutput['productTotal'] = moneyFormat('%(#1n', $dblCurrentProductTotal);
