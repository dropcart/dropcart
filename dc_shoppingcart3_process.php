...
while ($objStatus = $objDB->getObject($result2)) {
    ...
    if ((isset($payment) && $payment->isPaid() == TRUE) || $objStatus->status == 'ready') {
        ...
        $strSQL = "INSERT INTO " . DB_PREFIX . "customers_orders (uuid, orderId, custId, entryDate, firstname, lastname, company, address, houseNr, houseNrAdd, zipcode, city, lang, delFirstname, delLastname, delCompany, delAddress, delHouseNr, delHouseNrAdd, delZipcode, delCity, delLang, email, paymethodId, ip) " .
        "VALUES ('" . $intUuid . "', " . $intOrderId . ", " . $objCustomer->customerId . ", NOW(), '" . addslashes($objCustomer->firstname) . "', '" . addslashes($objCustomer->lastname) . "', '" . addslashes($objCustomer->company) . "', '" . addslashes($objCustomer->address) . "', '" . addslashes($objCustomer->houseNr) . "', '" . addslashes($objCustomer->houseNrAdd) . "', '" . addslashes($objCustomer->zipcode) . "', '" . addslashes($objCustomer->city) . "', '" . addslashes($objCustomer->lang) . "', '" . addslashes($objCustomer->delFirstname) . "', '" . addslashes($objCustomer->delLastname) . "', '" . addslashes($objCustomer->delCompany) . "', '" . addslashes($objCustomer->delAddress) . "', '" . addslashes($objCustomer->delHouseNr) . "', '" . addslashes($objCustomer->delHouseNrAdd) . "', '" . addslashes($objCustomer->delZipcode) . "', '" . addslashes($objCustomer->delCity) . "', '" . addslashes($objCustomer->delLang) . "', '" . $objCustomer->email . "', " . $intPayMethodId . ", '" . ip() . "')";
        $result = $objDB->sqlExecute($strSQL);
