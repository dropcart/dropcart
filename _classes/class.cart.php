...
class Cart {
    ...
    function getCart($archived = false, $orderId = 0) {
        ...
        if (!$archived) {
            if (!empty($this->intCustomerId)) {
                $strExtraSQL = "AND c.customerId=" . $this->intCustomerId;
            } else {
                $strExtraSQL = "AND c.sessionId='" . $this->intSessionId . "'";
            }
    ...
    function updateCart() {
        ...
        while ($objProduct = $this->objDB->getObject($result)) {
            ...
            //product
            if ($intQuantity > 0) {
                $this->updateProductQuantity($objProduct->id, $intQuantity);
            }
            //update
            else {
                $this->deleteProduct($objProduct->id);
            }
            //delete
        }
    }
